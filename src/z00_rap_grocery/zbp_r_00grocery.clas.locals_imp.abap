CLASS LHC_ZR_00GROCERY DEFINITION INHERITING FROM CL_ABAP_BEHAVIOR_HANDLER.
  PRIVATE SECTION.
    METHODS:
...
CLASS LHC_ZR_00GROCERY IMPLEMENTATION.
  METHOD GET_GLOBAL_AUTHORIZATIONS.
  ENDMETHOD.

  METHOD checkExpirationDate.

    DATA: lt_groceries TYPE TABLE FOR READ RESULT zr_00grocery,
          ls_grocery TYPE STRUCTURE FOR READ RESULT zr_00grocery,
          lv_expiration TYPE d,
          lv_current_date TYPE d,
          lv_expired TYPE abap_boolean,
          lt_update_groceries TYPE TABLE FOR UPDATE zr_00grocery.

    READ ENTITIES OF zr_00grocery
    ...
    LOOP AT lt_groceries INTO ls_grocery.
        ...
        MODIFY ENTITIES OF zr_00grocery IN LOCAL MODE
            ENTITY Grocery
    ...
    result = VALUE #(
        FOR groceries IN lt_groceries ( id = groceries-id %param = groceries )
        ).
